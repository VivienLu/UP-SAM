...
class MLPBlock3D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.lin1 = lora.Linear(embedding_dim, mlp_dim, r=ft_r)
        self.lin2 = lora.Linear(mlp_dim, embedding_dim, r=ft_r)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.q_proj = lora.Linear(embedding_dim, self.internal_dim, r=ft_r)
        self.k_proj = nn.Linear(embedding_dim, self.internal_dim)
        self.v_proj = lora.Linear(embedding_dim, self.internal_dim, r=ft_r)
        self.out_proj = lora.Linear(self.internal_dim, embedding_dim, r=ft_r)
...
class MaskDecoder3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.iou_token = lora.Embedding(1, transformer_dim, r=ft_r) # 384
        self.num_mask_tokens = num_multimask_outputs + 1
        self.mask_tokens = lora.Embedding(self.num_mask_tokens, transformer_dim, r=ft_r) # 4 * 384
...
class MLP(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            lora.Linear(n, k, r=ft_r) for n, k in zip([input_dim] + h, h + [output_dim])
        )
